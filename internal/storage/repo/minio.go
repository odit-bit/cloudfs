...
func (s *MinioAdapter) Put(ctx context.Context, bucketName, key string, file io.Reader, size int64, contentType string) (*storage.ObjectInfo, error) {
...
func (s *MinioAdapter) Get(ctx context.Context, bucketName, filename string) (*storage.ObjectInfo, error) {
	...
	var objInfo storage.ObjectInfo
...
func (s *MinioAdapter) Info(ctx context.Context, bucketName, fileName string) (*storage.ObjectInfo, error) {
	stat, err := s.minioCli.StatObject(ctx, bucketName, fileName, minio.GetObjectOptions{})
	if err != nil {
		return nil, err
	}
...
func (s *MinioAdapter) GetShareUrl(bucket, key string) storage.ShareFunc {
...
func (s *MinioAdapter) ObjectIterator(ctx context.Context, bucketName string, limit int, lastFilename string) storage.Iterator {
	...
	objC := make(chan storage.ObjectInfo)
