...
func Test_Object(t *testing.T) {
	...
	// ListObject  empty list should not error
	assertListObject(ctx, t, client, headerObj)
	...
	// CreateShareToken
	shareToken, err := client.ShareObject(ctx, &apipb.ShareObjectRequest{
	...
	// CreateShareToken should idempoten
	shareToken2, err := client.ShareObject(ctx, &apipb.ShareObjectRequest{
		Token:    headerObj["authorization"],
		Filename: headerObj["filename"],
	})
	if err != nil {
	...
	_ = shareToken2
	assert.Equal(t, shareToken, shareToken2)
...
func assertUploadObject(ctx context.Context, t *testing.T, client apipb.StorageServiceClient, data []byte, headerObj map[string]string) {
...
func assertDownloadObject(ctx context.Context, t *testing.T, client apipb.StorageServiceClient, data []byte, headerObj map[string]string) {
...
func assertListObject(ctx context.Context, t *testing.T, client apipb.StorageServiceClient, expected map[string]string) {
	res, err := client.ListObject(ctx, &apipb.ListObjectRequest{UserToken: expected["authorization"], Limit: 100})
	if err != nil {
		t.Fatal(err)
	}
